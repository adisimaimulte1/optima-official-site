...
export default function FrameAnimation({
  ...
  const finalYRef = useRef(null); // not 0, so we can use 0 as a valid value
  ...
  // Once linear motion ends, transition to float
  if (frame === frameCount - 1) {
    if (finalYRef.current === null) {
      finalYRef.current = linearY;
      setFloatStartTime(performance.now()); // ⏱ mark float start
    }
    ...
    const floatOffset =
      Math.sin(timeSinceFloat / 1000 * 2 * Math.PI * floatFrequency) * floatAmplitude;
